...
<jsp:include page="bm_sidebar.jsp" flush="false" />
	<div id="main-contents" >
		...
		<tbody>
			...
			<c:forEach var="bmdto"  items="${cHzBmBGnoList}">
				<tr>
                        <th style="width: 40px;"><input id="checkAll" name="" onclick="selectAllTodo()" type="checkbox" value="" title="checkAll"></th>
				...
				<table>
					<c:if test="${count > 0}">
						...
						<c:if test="${startPage > 10}">
							<a href="cHzBmBGnoList.jp?pageNum=${startPage-10}" >[이전]</a>
						</c:if>
			
						<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
							<a href="cHzBmBGnoList.jp?pageNum=${i}">${i}&nbsp;</a>
						</c:forEach>
		
						<c:if test="${endPage < pageCount}">
							<a href="cHzBmBGnoList.jp?pageNum=${startPage+10}">[다음]</a>
						</c:if>
					</c:if>
				</table>
			</div>
