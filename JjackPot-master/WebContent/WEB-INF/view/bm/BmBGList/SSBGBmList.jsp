...
<jsp:include page="bm_sidebar.jsp" flush="false" />
	<div id="main-contents">
		...
		<tbody>
			...
			<c:forEach var="bmdto"  items="${SSBmBgList}">
				<tr>
                        ...
                        <th style="min-width: 200px;">
                       		<a href="SSBmBGContent.jp?bm_num=${bmdto.bm_num}&pageNum=${pageNum}">
                            	${bmdto.bm_title}
                            </a>	
                        </th>
                        <th style="width: 120px;"></th>
				...
				<table>
					<c:if test="${count > 0}">
						...
						<c:if test="${startPage > 10}">
							<a href="SSBGBmList.jp?pageNum=${startPage-10}" >[이전]</a>
						</c:if>
			
						<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
							<a href="SSBGBmList.jp?pageNum=${i}" style="text-align: center; font-size: 13;">${i}&nbsp;</a>
						</c:forEach>
		
						<c:if test="${endPage < pageCount}">
							<a href="SSBGBmList.jp?pageNum=${startPage+10}">[다음]</a>
						</c:if>
					</c:if>
				</table>
			</div>
	        ...
	        <div class="btn-wrap">
	            ...
	            <button type="button" class="btn btn-color5 br" onclick="window.location='bmBG_state_update.jp?bm_num=${bm_num};">업무완료</button>
